/****************************************************************************
 *   $Id: timer16.c,v 1.1 2012/04/25 17:42:57 cvsuser Exp $
...
void delay_microseconds(uint8_t timer_num, uint32_t delayIn_us)
{
  if (timer_num == 0)
  {
    ...
    */
	...
	//LPC_TMR16B0->PR  = 71;			/* set prescaler to 72 = 1MHZ */
	LPC_TMR16B0->PR  = MHZ_PRESCALE; /* set prescaler to get 1 M counts/sec */
	LPC_TMR16B0->MR0 = delayIn_us;
...
void delayMs(uint8_t timer_num, uint32_t delayInMs)
{
  if (timer_num == 0)
  {
    ...
    */
    ...
    LPC_TMR16B0->IR  = 0xff;		/* reset all interrrupts */
    LPC_TMR16B0->MCR = 0x14;		/* stop timer on match */
    LPC_TMR16B0->TCR = 0x01;		/* start timer */
    /* wait until delay time has elapsed */
    while (LPC_TMR16B0->TCR & 0x01);
  }
  else if (timer_num == 1)
  {
    ...
    */
    LPC_TMR16B1->TCR = 0x02;		/* reset timer */
    LPC_TMR16B1->PR  = MHZ_PRESCALE * TIME_INTERVALmS; /* set prescaler to get 1 M counts/sec */
    LPC_TMR16B1->MR0 = delayInMs;
    LPC_TMR16B1->IR  = 0xff;		/* reset all interrrupts */
    LPC_TMR16B1->MCR = 0x04;		/* stop timer on match */
    LPC_TMR16B1->TCR = 0x01;		/* start timer */
    /* wait until delay time has elapsed */
